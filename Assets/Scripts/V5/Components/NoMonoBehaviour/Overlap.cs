...
public class Overlap
{
    ...
    public bool EnemyOverlapping(Transform transform, LayerMask enemy) =>
        Physics2D.OverlapBox((Vector2)transform.localPosition + (transform.localScale.x > 0 ? enemyDetectorPos : new Vector2(-1, 1) * enemyDetectorPos), enemyDetectorSize, 0f, enemy) != null;
    ...
    public void DrawEnemyOverlapping(Transform transform, LayerMask layer)
    {
        if (EnemyOverlapping(transform, layer))
            Gizmos.color = Color.green;
        else
            Gizmos.color = Color.red;

        Gizmos.DrawWireCube((Vector2)transform.localPosition + (transform.localScale.x > 0 ? enemyDetectorPos : new Vector2(-1, 1) * enemyDetectorPos), enemyDetectorSize);
    }
