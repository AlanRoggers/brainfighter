...
public class Overlap
{
    ...
    public bool EnemyOverlapping(Collider2D collider) =>
        Physics2D.OverlapBox(
            new Vector2(collider.bounds.center.x + (collider.transform.localScale.x > 0 ? 0.3f : -0.3f), collider.bounds.center.y),
            enemyDetectorSize,
            0,
            collider.gameObject.layer == 6 ? LayerMask.GetMask("Player2") : LayerMask.GetMask("Player1")) != null;
